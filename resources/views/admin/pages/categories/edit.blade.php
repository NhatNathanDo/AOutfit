...
<div class="flex items-center justify-between mb-4">
  <h2 class="text-xl font-semibold">Sửa danh mục</h2>
...
<form action="{{ route('admin.categories.update', $category->id) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
  @csrf
  @method('PUT')
  <div class="form-control md:col-span-1">
    <label class="label"><span class="label-text">Tên danh mục</span></label>
    <input type="text" name="name" class="input input-bordered" value="{{ old('name', $category->name) }}" required />
  </div>
  <div class="form-control md:col-span-1">
    <label class="label"><span class="label-text">Slug (tùy chọn)</span></label>
    <input type="text" name="slug" class="input input-bordered" value="{{ old('slug', $category->slug) }}" />
  </div>
